...
describe('normaliseName', () => {
  describe('form extraction', () => {
    it('should extract fresh form from "fresh garlic"', () => {
      ...
      expect(result).toEqual({ canonical: 'garlic (fresh)', base: 'garlic', form: 'fresh', confident: true })
    })
    ...
    it('should extract tinned form from "tinned tomatoes"', () => {
      ...
      expect(result).toEqual({ canonical: 'tomato (tinned)', base: 'tomato', form: 'tinned', confident: true })
    })
    ...
    it('should extract dried form from "garlic granules"', () => {
      ...
      expect(result).toEqual({ canonical: 'garlic (dried)', base: 'garlic', form: 'dried', confident: true })
    })

    it('should extract dried form from "dried oregano"', () => {
      const result = normaliseName('dried oregano')
      expect(result).toEqual({ canonical: 'oregano (dried)', base: 'oregano', form: 'dried', confident: true })
    })

    it('should extract ground form from "ground cumin"', () => {
      const result = normaliseName('ground cumin')
      expect(result).toEqual({ canonical: 'cumin (ground)', base: 'cumin', form: 'ground', confident: true })
    })

    it('should extract paste form from "tomato paste"', () => {
      const result = normaliseName('tomato paste')
      expect(result).toEqual({ canonical: 'tomato (paste)', base: 'tomato', form: 'paste', confident: true })
    })

    it('should extract flaked form from "flaked almonds"', () => {
      const result = normaliseName('flaked almonds')
      expect(result).toEqual({ canonical: 'almond (flaked)', base: 'almond', form: 'flaked', confident: true })
    })

    it('should extract frozen form from "frozen peas"', () => {
      const result = normaliseName('frozen peas')
      expect(result).toEqual({ canonical: 'pea (frozen)', base: 'pea', form: 'frozen', confident: true })
    })

    it('should extract smoked form from "smoked tofu"', () => {
      const result = normaliseName('smoked tofu')
      expect(result).toEqual({ canonical: 'tofu (smoked)', base: 'tofu', form: 'smoked', confident: true })
    })
  })
  ...
  describe('no form (pass-through)', () => {
    it('should pass through "chicken breast" without form', () => {
      ...
      expect(result).toEqual({ canonical: 'chicken breast', base: 'chicken breast', form: null, confident: false })
    })

    it('should pass through "olive oil" without form', () => {
      const result = normaliseName('olive oil')
      expect(result).toEqual({ canonical: 'olive oil', base: 'olive oil', form: null, confident: false })
    })

    it('should pass through "soy sauce" without form', () => {
      const result = normaliseName('soy sauce')
      expect(result).toEqual({ canonical: 'soy sauce', base: 'soy sauce', form: null, confident: false })
    })

    it('should singularise "spring onions" to "spring onion"', () => {
      const result = normaliseName('spring onions')
      expect(result).toEqual({ canonical: 'spring onion', base: 'spring onion', form: null, confident: false })
    })
  })
  ...
  describe('compound exceptions', () => {
    it('should treat "ground beef" as a base, not form=ground', () => {
      ...
      expect(result).toEqual({ canonical: 'ground beef', base: 'ground beef', form: null, confident: true })
    })

    it('should treat "smoked salmon" as a base, not form=smoked', () => {
      const result = normaliseName('smoked salmon')
      expect(result).toEqual({ canonical: 'smoked salmon', base: 'smoked salmon', form: null, confident: true })
    })

    it('should treat "smoked paprika" as a base, not form=smoked', () => {
      const result = normaliseName('smoked paprika')
      expect(result).toEqual({ canonical: 'smoked paprika', base: 'smoked paprika', form: null, confident: true })
    })
  })
  ...
  describe('singularisation', () => {
    it('should singularise "tomatoes" to "tomato"', () => {
      ...
      expect(result).toEqual({ canonical: 'tomato', base: 'tomato', form: null, confident: false })
    })

    it('should singularise "potatoes" to "potato"', () => {
      const result = normaliseName('potatoes')
      expect(result).toEqual({ canonical: 'potato', base: 'potato', form: null, confident: false })
    })

    it('should singularise "carrots" to "carrot"', () => {
      const result = normaliseName('carrots')
      expect(result).toEqual({ canonical: 'carrot', base: 'carrot', form: null, confident: false })
    })
  })
  ...
  describe('edge cases', () => {
    it('should handle empty string', () => {
      ...
      expect(result).toEqual({ canonical: '', base: '', form: null, confident: false })
    })
